package com.example.myappkotlin.ui.customviews
...
import com.example.myappkotlin.data.Color
...
class ColorPickerView : LinearLayout {
        ...
        Color.values().forEach { color ->
            addView(
                ColorCircleView(context).apply {
                    fillColorRes = color.mapToColor(context)
                    tag = color
    ...
    fun close() {
        animator.cancel()
        animator.setValues(
            PropertyValuesHolder.ofInt(HEIGHT, measuredHeight, 0), PropertyValuesHolder.ofFloat(
                SCALE, getChildAt(0).scaleX, 0f
            )
        )
        animator.start()
    }
